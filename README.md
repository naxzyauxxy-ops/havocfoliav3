...
| Path | What |
| --- | --- |
| `fork-code/` | Feature code, staged off the compile path — see `fork-code/README.md` |
| `havocfolia-server/minecraft-patches/sources/` | NMS hooks, generated by `rebuildAllServerPatches` |
| `scripts/apply-hooks.py` | Inserts the hooks into generated sources so Gradle can write the patches |
